 ...
 *
 * @param kind             判定出的体裁
 * @param originalTokens   压缩前 token 数（口径见 {@link #counter}）
 * @param compressedTokens 压缩后 token 数（口径见 {@link #counter}）
 ...
 * @param counter          上面那两个 token 数是按什么口径算的（真实词表 / 启发式估算）。
 *                         同一份内容在两种口径下能差 60%，不写清楚的话
 *                         "压缩到 6,000 token" 这句话本身就不成立。
 */
public record PressReport(
    ...
    public PressReport {
        ...
        Objects.requireNonNull(counter, "counter：报告必须说明 token 数字的口径");
    }

    /** 满足预算的构造（绝大多数情况） */
    public PressReport(ContextKind kind, int originalTokens, int compressedTokens, double reductionPercent,
                       int protectedSegments, List<String> actions, TokenCounterInfo counter) {
        this(kind, originalTokens, compressedTokens, reductionPercent, protectedSegments, actions, 0, counter);
    }
    ...
    public String summary() {
        ...
        sb.append("%s: %d -> %d tokens (-%.1f%%), 保护 %d 段, 计数口径=%s"
