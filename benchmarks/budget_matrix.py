...
判定口径（三分类）：
  ...
  - ok            压缩后 <= 预算
  - declared      压缩后 > 预算，但报告显式声明了不可满足。两种成因都算：
                  ① 受保护内容本身就超预算；② 压根没有可裁的结构（字段一个不能少、
                  字符串都在截断阈值以内、没有数组可采样）。
                  两者对调用方的出口不同（放宽保护额度 vs 提高预算），
                  所以报告里的动作名把它们分开了——这一格只看"有没有如实声明"。
    ...
    for name in CORPORA:
        ...
        for budget in BUDGETS:
            ...
            if orig <= budget:
                ...
                # 压缩器如实声明了"不可满足"——按契约算合规，单独计数。
                # 具体成因（受保护内容超预算 / 没有可裁的结构）看 `动作=` 里的
                # NOTHING_LEFT_TO_TRIM_EXCEPT_PROTECTED_VALUES 还是 NO_TRIMMABLE_STRUCTURE_LEFT。
                verdict = "declared"
            else:
                verdict = "violated"
            rows.append({
