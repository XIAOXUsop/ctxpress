...
- **未保护部分可经内容寻址归档逐字节取回**；**归档引用通常会被写进压缩内容本身**，
  但有一个例外见下。
- **输出不含原文没有的内容。** 这句话按档位成立，两档的线不一样：

  | 档位 | 承诺 | 实测边界 |
  |---|---|---|
  | 一：仅去空白（`MINIFIED_ONLY`） | 逐**字节**来自输入 | 数值字面量、Unicode 转义、尾随零一律原样 |
  | 二/三：采样 / 截断 | 数值逐**值**不动，结构按设计有损 | 数值的**拼写**会被重新序列化归一：`1e400` → `1E+400`、`1e-400` → `1E-400`（值不变） |

  标记文法（日志：`…[省略 N 行]…`、折叠 `<原行>   [重复 N 次]`；文本：`…[省略 N 句]…`；
  JSON：`…[N 字符已省略]…` / `{"_omitted":N}`；归档引用：`_ctxpress_archive`）
  两个档里都算"已声明的内容"，不记为凭空生成。

  > **为什么第二、三档只保"值"不保"字节"**：那两档要按预算裁结构，走的是解析再序列化。
  > 数值经 BigDecimal 能精确承载**值**，但拼写由 `BigDecimal.toString()` 决定，
  > 指数形式的字面会被归一。这不是缺陷，是这两档写明的边界——
  > 要连字节都不动，请走第一档（最小化结果装得下时自动走它）。
  > 保真证书按这个分寸分档判：档位一比**字面**，档位二/三用 `Decimal.compare_total` 比**值**。
  > 后者不比前者松，反而更严——`100.00` → `100.0` 这种尾随零丢失，
  > 字面比法会漏（`"100.0"` 是输入 `100.00` 的子串），比值才抓得住。
  >
  > ⚠️ **这段契约为它当初的措辞付过代价。** 这里原先写的是"每个字节要么逐字来自输入"，
  > 无条件、不区分档位；而漏掉 JSON 改写缺陷的盲区正在这里：
  > 保真证书当时只覆盖日志那一类，`benchmarks/fidelity.py` 的 `MARKER` 只认 `…[省略 N 行]…`，
  > 两组语料也都是日志——**会凭空造字节的那条路径恰好不在证书的覆盖里**。
  > 现在证书覆盖了日志 / 折叠 / JSON / TEXT / 数值五组（2026-09-22）。
  > 补的过程中，"数值"那组本身被三次发现是空的（语料字段被截光、夹具写成了字符串、
  > 缩进太浅导致任何合法预算下压缩器都直接 NO_OP）——**空检查比没有检查更坏**，
  > 它让人以为量过了。三次的细节都写在 `check_number_literals` 的文档串里。
...
- **数值的"值"在任何一档都不会被改写（2026-09-22 起）。** 第二、三档按设计是有损的
  （要截断、要采样），但"截断"该截的是结构，不是数字本身。Jackson 默认把小数读成
  double，实测 `998877665544332211.99` → `9.988776655443322E17`、`1e400` → `"Infinity"`、
  `1e-400` → `0.0`，**而报告里没有任何一条动作说数值被改过**。
  现在小数一律走 BigDecimal 精确承载（`1e400` 与 `1e-400` 都能原样装下）。
  注意分寸：这两档保的是**值**，不是字节——第一档才连字节都不动。
...
**保真证书，五组全 0：** 日志语料 6 个用例共 0 行凭空生成、折叠路径 0 行、
JSON 语料 0 处、数值 0 处被改写、TEXT 语料 0 段。输出的每一个字节要么逐字来自输入
（数值那组按档位比字节或比值），要么匹配已声明的省略标记文法——这是抽取式压缩
相对摘要式压缩的硬优势（ACL 2020 的测量显示 >70% 的单句摘要含幻觉）。

五组必须分开看，因为它们覆盖的是**不同的代码路径**：日志 / 折叠 / JSON / TEXT / 数值。
前两组之外的三组是 2026-09-22 才补的——补之前，会凭空造字节的那条路径
（JSON 的数值改写）恰好不在证书的覆盖里，证书天天绿。
...
> ✅ **v0.4.4 修掉了 v0.4.3 的两处问题**，下载最新版即可：
>
> | v0.4.3 的问题 | v0.4.4 |
> |---|---|
> | 「仅去空白」那一档其实是**解析再序列化**，会静默改写数值字面量：`1e400` → `"Infinity"`、`99999999999999999999.99` → `1.0E20`、`1e-400` → `0.0`。而报告只写 `MINIFIED_ONLY`、归档引用是 null，**没有任何标记说值被改过** | 这一档换成纯文本扫描（`c1f53d3`），输出逐字节来自输入 |
> | 根是数组、只截了内层字符串时，归档引用没写进内容——那条路径上「读到内容就能取回」是假的 | 改成写进第一条字符串省略标记（`5ea19c7`） |
> 还需要知道的两点（不是 v0.4.3 独有，是**至今仍存在**的边界）：根是数组且**只截了内层数组**、全程没有字符串可承载时，引用仍进不了内容（动作名会明说 `ARCHIVE_REF_NOT_EMBEDDABLE`）；第二、三档重排后数值的**拼写**会归一（`1e400` → `1E+400`，**值不变**），要连字节都不动请走第一档。两条都写在下面「契约」一节里。
>
> **v0.4.3 当时修的是 v0.4.2 的两处**：`retrieve` 取回多写 1 个字节（「逐字节一致」不成立），
> 以及内嵌 jackson-databind 2.19.0（命中 5 条公告，2 HIGH）。那两条在 0.4.3 里就是好的，本次没有回退。
>
> v0.4.2 是「产物落后于 master」的实例：它的 jar 里是修复之前的代码，而 master 上早已修好。
> 现在 `release.yml` 里有两道发版闸（依赖告警 + tag 是否落后且动了构建配置），
> 这类情况不会再静默发出——**但闸只管"发的时候别发旧的"，管不了"发完之后 master 又修了"**，
> 那只能靠发版这件事本身被记起来（v0.4.3 → v0.4.4 之间隔了 4 个提交，其中两个是真修复）。
>
> 如果你确实要用更旧的版本：
> - v0.4.2：**别信它的「逐字节一致」**——取回的文件会比原文多一个换行字节；
>   它的内嵌 Jackson 建议在宿主工程里显式钉 `jackson-bom` ≥ 2.21.5。
> - v0.4.3：**别拿它压带数值的 JSON**——「仅去空白」那一档会把值改掉且不声明。
...
```bash
./mvnw test      # 104 项，全部离线（core 78 + tokenizer 11 + cli 15）
```
