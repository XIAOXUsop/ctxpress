...
12. **`retrieve` 取回的文件永远比原文多一个换行** —— 命令行取回时无条件
    `out.print('\n')`，于是「逐字节一致」这条承诺**从来就不成立**：
    实测 4000 行日志取回后 251628 字节 vs 原文 251627，sha256 对不上。
    而归档引用 `ORIG-xxx` 正是原文内容的 sha256 前缀，**用户拿它一校验就会发现**。
    这条之所以长期隐身，是因为回归用例写的是
    `assertEquals(original, retrieved.out().stripTrailing(), "必须与原文逐字节相同")`
